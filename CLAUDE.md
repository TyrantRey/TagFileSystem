...
**Tag parsing (`services/tagging.py`, `core/interface/tag.py`)** — `TaggingParser` is registry-driven, not if/else: `register(prefix, field)` is a decorator mapping a marker prefix to a `TagParserOutput` field and a `str -> model` factory; the regex is built from the registered prefixes. `--` → `tags` (`Tag(name=...)`) and `@@` → `actions` (`TagAction(name=...)`) are registered in `__init__`; re-registering a prefix replaces its factory. `parse(str)` looks each match up in the table and appends to the field's list. `Tag.name` is normalized (lowercase, non-`\w-` stripped, hyphens collapsed, and a leading `category:` prefix dropped). `TagAction` parses `name:k=v,k2=v2` into `params`. Markers that fail validation (e.g. a trailing `--`) are logged and skipped instead of aborting the parse. The DB pipeline calls the parser on `path.stem`.
